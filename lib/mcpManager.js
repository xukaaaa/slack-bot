 ...
 * @returns {Promise<Object>} Tool result
 */
export async function callDynamicMCPTool(toolKey, args = {}) {
...
/**
 * Generate AI tools from MCP servers
 * @returns {Promise<Array>} AI tool definitions
 */
export async function generateAIToolsFromMCP() {
  const toolRegistry = await discoverAllMCPTools();
  const aiTools = [];

  for (const [toolKey, toolInfo] of Object.entries(toolRegistry)) {
    aiTools.push({
      type: 'function',
      function: {
        name: toolKey,
  ...
  console.log('[MCP Manager] Generated', aiTools.length, 'AI tools');
  return aiTools;
}

/**
 * Call MCP tool
 * @param {string} toolName - Tool name (format: "serverName_toolName")
 * @param {Object} args - Tool arguments
 * @returns {Promise<Object>} Tool result
 */
export async function callMCPTool(toolName, args = {}) {
  return await callDynamicMCPTool(toolName, args);
}

...
/**
 * Helper function to lookup user ID by username
 * @param {string} username - Username to lookup
 * @param {string} redmineUrl - Redmine base URL
 * @param {string} redmineApiKey - API key
 * @param {string} projectId - Project ID
 * @returns {Promise<number|null>} User ID or null if not found
 */
async function lookupUserId(username, redmineUrl, redmineApiKey, projectId) {
  try {
    // Get project members (more reliable than users API which may need admin rights)
    ...
    if (!response.ok) {
      console.error('[REDMINE] Failed to fetch project members');
      return null;
    }
    ...
    const member = memberships.find(m =>
      m.user && m.user.name.toLowerCase() === username.toLowerCase()
    );

    return member ? member.user.id : null;
  } catch (error) {
    console.error('[REDMINE] Error looking up user ID:', error);
    return null;
  }
}
...
export async function executeUpdateRedmineIssue(args) {
  ...
  try {
    // Resolve assigned_to_id if it's a username
    let resolvedAssignedToId = assigned_to_id;
    if (assigned_to_id && isNaN(assigned_to_id)) {
      console.log(`[REDMINE] Looking up user ID for username: ${assigned_to_id}`);
      const projectId = process.env.REDMINE_DEFAULT_PROJECT_ID;
      const userId = await lookupUserId(assigned_to_id, redmineUrl, redmineApiKey, projectId);
      if (userId) {
        resolvedAssignedToId = userId;
        console.log(`[REDMINE] Resolved username '${assigned_to_id}' to user ID: ${userId}`);
      } else {
        console.warn(`[REDMINE] Could not find user ID for username: ${assigned_to_id}`);
        return {
          success: false,
          message: `Không tìm thấy user với username: ${assigned_to_id}`
        };
      }
    }
    ...
    if (resolvedAssignedToId !== undefined) issueData.issue.assigned_to_id = resolvedAssignedToId;
...
/**
 * Get list of issues from Redmine with tree structure (including children)
 * @param {Object} args - Query parameters
 * @param {string} args.status - Filter by status: 'open', 'closed', '*' (all), or status_id
 * @param {string} args.assigned_to_id - Filter by assigned user: user_id or 'me' or username
 ...
 * @returns {Promise<Object>} Result object with issues list in tree structure
 */
export async function executeGetRedmineIssues(args) {
  const {
    status = 'open',
    assigned_to_id,
    limit = 100,  // Default to 100 to get more tasks
  ...
  try {
    // If assigned_to_id is a username (string, not 'me'), lookup user ID
    let resolvedAssignedToId = assigned_to_id;
    if (assigned_to_id && assigned_to_id !== 'me' && isNaN(assigned_to_id)) {
      console.log(`[REDMINE] Looking up user ID for username: ${assigned_to_id}`);
      const userId = await lookupUserId(assigned_to_id, redmineUrl, redmineApiKey, projectId);
      if (userId) {
        resolvedAssignedToId = userId;
    ...
    const params = new URLSearchParams({
      ...
      sort: sort,
      include: 'children' // Include subtasks
    });

    // Add optional filters
    if (resolvedAssignedToId) {
      params.append('assigned_to_id', resolvedAssignedToId);
    }
    ...
    const formatIssue = (issue) => ({
      ...
      children: issue.children ? issue.children.map(formatIssue) : []
    });

    // Format issues with tree structure
...
export async function executeCreateRedmineIssue(args) {
  ...
  try {
    ...
    if (resolvedAssignedToId) {
      issueData.issue.assigned_to_id = resolvedAssignedToId;
    }

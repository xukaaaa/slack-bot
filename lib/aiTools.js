...
function getRedmineTools() {
  return [
    ...
    // MCP Redmine Tool: List Project Members
    {
      type: 'function',
      function: {
        name: 'listProjectMembers',
        description: 'Lấy danh sách TẤT CẢ thành viên trong project với roles và thông tin chi tiết. Dùng để tìm user ID khi cần assign task.',
        parameters: {
          type: 'object',
          properties: {
            project_id: {
              type: ['number', 'string'],
              description: 'Project ID (number) hoặc project identifier (string)'
            }
          },
          required: ['project_id']
        }
